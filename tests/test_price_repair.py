            ...
            for i in range(len(intervals)):
                ...
                for j in range(i, len(periods)):
                    ...
                    if len(dfr) != len(df_truth):
                        ...
                        vol_diff_pct0 = (dfr["Volume"].iloc[0] - df_truth["Volume"].iloc[0]) / df_truth["Volume"].iloc[
                            0
                        ]
                        vol_diff_pct1 = (dfr["Volume"].iloc[-1] - df_truth["Volume"].iloc[-1]) / df_truth[
                            "Volume"
                        ].iloc[-1]
...
class TestPriceRepair(unittest.TestCase):
    ...
    def test_repair_100x_random_weekly(self):
        ...
        df = _pd.DataFrame(
            ...
            index=_pd.to_datetime(
                [_dt.date(2022, 10, 24), _dt.date(2022, 10, 17), _dt.date(2022, 10, 10), _dt.date(2022, 10, 3)]
            ),
        )
        df = df.sort_index()
        df.index.name = "Date"
    ...
    def test_repair_100x_random_weekly_preSplit(self):
        ...
        df = _pd.DataFrame(
            ...
            index=_pd.to_datetime(
                [_dt.date(2020, 3, 30), _dt.date(2020, 3, 23), _dt.date(2020, 3, 16), _dt.date(2020, 3, 9)]
            ),
        )
        df = df.sort_index()
    ...
    def test_repair_100x_random_daily(self):
        ...
        df = _pd.DataFrame(
            ...
            index=_pd.to_datetime(
                [_dt.date(2022, 11, 1), _dt.date(2022, 10, 31), _dt.date(2022, 10, 28), _dt.date(2022, 10, 27)]
            ),
        )
        for c in data_cols:
    ...
    def test_repair_zeroes_daily(self):
        ...
        df_bad = _pd.DataFrame(
            ...
            index=_pd.to_datetime([_dt.datetime(2025, 3, 17), _dt.datetime(2025, 3, 14), _dt.datetime(2025, 3, 13)]),
        )
        df_bad = df_bad.sort_index()
    ...
    def test_repair_zeroes_daily_adjClose(self):
        ...
        df = _pd.DataFrame(
            data={
                "Open": [2.020000e01, 2.032000e01, 1.992000e01, 1.910000e01, 2.008000e01],
                "High": [2.039000e01, 2.063000e01, 2.025000e01, 2.055000e01, 2.015000e01],
                "Low": [1.929000e01, 1.975000e01, 1.895000e01, 1.884000e01, 1.950000e01],
                "Close": [2.011000e01, 1.983000e01, 1.899000e01, 2.049000e01, 1.971000e01],
                "Adj Close": [1.998323e01, 1.970500e01, 1.899000e01, 2.049000e01, 1.971000e01],
                "Volume": [1.473857e08, 1.066704e08, 9.797230e07, 9.683680e07, 7.639450e07],
                "Dividends": [0.000000e00, 0.000000e00, 1.250000e-01, 0.000000e00, 0.000000e00],
            },
            index=_pd.to_datetime(
                [
