...
class Analysis:
    ...
    def _get_periodic_df(self, key, currency_key=None) -> pl.DataFrame:
    ...
    @property
    def earnings_estimate(self) -> pl.DataFrame:
        if self._earnings_estimate is not None:
            return self._earnings_estimate
        self._earnings_estimate = self._get_periodic_df(
            "earningsEstimate", currency_key="earningsCurrency"
        )
        return self._earnings_estimate

    @property
    def revenue_estimate(self) -> pl.DataFrame:
        if self._revenue_estimate is not None:
            return self._revenue_estimate
        self._revenue_estimate = self._get_periodic_df(
            "revenueEstimate", currency_key="revenueCurrency"
        )
        return self._revenue_estimate

    @property
    def eps_trend(self) -> pl.DataFrame:
        if self._eps_trend is not None:
            return self._eps_trend
        self._eps_trend = self._get_periodic_df(
            "epsTrend", currency_key="epsTrendCurrency"
        )
        return self._eps_trend

    @property
    def eps_revisions(self) -> pl.DataFrame:
        if self._eps_revisions is not None:
            return self._eps_revisions
        self._eps_revisions = self._get_periodic_df(
            "epsRevisions", currency_key="epsRevisionsCurrency"
        )
        return self._eps_revisions
    ...
    @property
    def earnings_history(self) -> pl.DataFrame:
        ...
        try:
            ...
            if not YfConfig.debug.hide_exceptions:
                raise
            self._earnings_history = pl.DataFrame()
            return self._earnings_history
    ...
    @property
    def growth_estimates(self) -> pl.DataFrame:
        ...
        try:
            ...
            if not YfConfig.debug.hide_exceptions:
                raise
            self._growth_estimates = pl.DataFrame()
            return self._growth_estimates
        ...
        df = pl.DataFrame(data)
        # Drop rows where all non-'period' values are null (replaces .dropna(how='all'))
