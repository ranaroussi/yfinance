...
class PriceHistory:
    ...
    def history(
        ...
        if data is None or not isinstance(data, dict):
            ...
            period and period not in self._history_metadata["validRanges"] and not utils.is_valid_period_format(period)
        ):
        ...
        df, last_trade = utils.fix_Yahoo_returning_live_separate(
            df, params["interval"], tz_exchange, prepost, repair=repair, currency=currency
        )
    ...
    def _reconstruct_intervals_batch(self, df, interval, prepost, tag=-1):
        ...
        if interval != self._reconstruct_start_interval and interval != nexts[self._reconstruct_start_interval]:
            msg = "Hit max depth of 2 ('{}'->'{}'->'{}')".format(
                self._reconstruct_start_interval, nexts[self._reconstruct_start_interval], interval
            )
                ...
                if f_tag.any():
                    ...
                    for idx in np.where(f_tag)[0]:
                        ...
                        if df_new.loc[dt, "Dividends"] != 0:
                            if idx < n - 1:
                                ...
                                div_adj = (
                                    1.0 - df_new_calib["Dividends"].iloc[idx] / df_new_calib["Close"].iloc[idx - 1]
                                )
                            ...
                            if idx > 0:
                                ...
                                if df_new_calib["Dividends"].iloc[idx + 1] != 0:
                                    div_adjusts.iloc[idx] *= (
                                        1.0 - df_new_calib["Dividends"].iloc[idx + 1] / df_new_calib["Close"].iloc[idx]
                                    )
                    ...
                    if f_close_bad.any():
                        ...
                        df_new.loc[div_adjusts_new_np, "Adj Close"] = (
                            df_new["Close"][div_adjusts_new_np] * div_adjusts_new[div_adjusts_new_np]
                        )
            ...
            if isclose(df2["Low"].iloc[div_idx], df2["Close"].iloc[div_idx - 1] * 100, rel_tol=0.025):
                ...
                if not isclose(present_adj, true_adjust, rel_tol=0.025):
                    df2.loc[: dt - _datetime.timedelta(seconds=1), "Adj Close"] = (
                        true_adjust * df2["Close"].loc[: dt - _datetime.timedelta(seconds=1)]
                    )
                            ...
                            if diff_fx < diff_fxPostSplit:
                                ...
                                if (diff_fxPostSplit * div_too_big_improvement_threshold) <= diff:
                                    possibilities.append(
                                        {"state": "div-too-big-and-pre-split", "diff": diff_fxPostSplit}
                                    )
                    ...
                    if drop_wo_vol > 0:
                        ...
                        if div_postSplit is None:
                            ...
                            if diff_fx < diff_fxPostSplit:
                                ...
                                if (diff_fxPostSplit * div_too_big_improvement_threshold) <= diff:
                                    possibilities.append(
                                        {"state": "div-too-small-and-pre-split", "diff": diff_fxPostSplit}
                                    )
            ...
            for c in cluster_checks:
                ...
                if c == "div_too_big":
                    ...
                    if (
                        ...
                        and (cluster[c] == (cluster[c] & cluster["adj_exceeds_prices"])).all()
                    ):
                    ...
                    if "div_exceeds_adj" in cluster.columns and cluster["div_exceeds_adj"].all():
                        ...
                        "adj_exceeds_prices" in cluster.columns and (cluster[c] == cluster["adj_exceeds_prices"]).all()
                    ):
    ...
    def _fix_bad_stock_splits(self, df, interval, tz_exchange):
        ...
        for split_idx in np.where(split_f)[0]:
            ...
            logger.debug(
                f"df dt range: {df_pre_split.index[0].date()} -> {df_pre_split.index[-1].date()}", extra=log_extras
            )
            df_pre_split_repaired = self._fix_prices_sudden_change(
                df_pre_split, interval, tz_exchange, split, correct_volume=True, correct_dividend=True
            )
            # Merge back in:
    ...
    def _fix_prices_sudden_change(
        self, df, interval, tz_exchange, change, correct_volume=False, correct_dividend=False
    ):
        if df.empty:
            return df
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            df_debug = df_debug.drop(
                ["Adj Close", "Volume", "Dividends", "Stock Splits", "Repaired?"], axis=1, errors="ignore"
            )
            debug_cols = ["Close"]
        ...
        if np.max(_1d_change_minx) < (split_max - 1) * 0.5 + 1 and np.min(_1d_change_minx) > 1.0 / (
            (split_max - 1) * 0.5 + 1
        ):
        ...
        if change in [100.0, 0.01] and f_splits.any():
            ...
            if f_pos.any():
                ...
                if isinstance(threshold, _dateutil.relativedelta.relativedelta) and isinstance(
                    gap_td, _dateutil.relativedelta.relativedelta
                ):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            if correct_columns_individually:
                f_change = df_debug[[c + "_down" for c in debug_cols]].any(axis=1) | df_debug[
                    [c + "_up" for c in debug_cols]
                ].any(axis=1)
            else:
                f_change = df_debug["down"] | df_debug["up"]
            f_change = (
                f_change | np.roll(f_change, -1) | np.roll(f_change, 1) | np.roll(f_change, -2) | np.roll(f_change, 2)
            )
        ...
        if correct_columns_individually:
            ...
            if appears_suspended and (idx_latest_active is not None and idx_latest_active >= idx_first_f):
                ...
                ranges_after = map_signals_to_ranges(
                    f_rev[idx_rev_latest_active:], f_rev_up[idx_rev_latest_active:], f_rev_down[idx_rev_latest_active:]
                )
                if len(ranges_after) > 0:
                    # Shift each range back to global indexing:
                    for i in range(len(ranges_after)):
            ...
            if start_min is not None:
                ...
                for i in range(len(ranges) - 1, -1, -1):
                    ...
                    if df2.index[r[0]].date() < start_min:
                        logger.debug(
                            f"Pruning range {df2.index[r[0]]}->{df2.index[r[1] - 1]} because too old.", extra=log_extras
                        )
                        del ranges[i]
            for r in ranges:
                if r[2] == "split":
