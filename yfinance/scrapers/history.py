...
class PriceHistory:
    ...
    def history(
        ...
        if start or period is None or period.lower() == "max":
            ...
            if start is None:
                ...
                if interval == "1m":
                    start = end - (SECONDS_IN_A_DAY * 8)  # 8 days / max chunk for 1m
                elif interval in ("2m", "5m", "15m", "30m", "90m"):
                    start = end - (SECONDS_IN_A_DAY * 60)  # 60 days / max chunk for these
                elif interval in ("1h", "60m"):
                    start = end - (SECONDS_IN_A_DAY * 730)  # 730 days / max chunk for 1h
                else:
                    start = end - (SECONDS_IN_A_YEAR * 99)  # 99 years / max chunk for day+ intervals
            else:
                start = utils._parse_user_dt(start, tz)

            # Program processing time adds seconds to request
            # To avoid exceeding max period, must subtract a buffer of seconds
            ARBITRARY_PROCESS_TIME_BUFFER = 60  # seconds
            start += ARBITRARY_PROCESS_TIME_BUFFER  # moves start forward in time
    ...
    def _fix_prices_sudden_change(self, df, interval, tz_exchange, change, correct_volume=False, correct_dividend=False):
        ...
        sd_pct = sd / avg
        logger.debug(
            f"Estimation of true 1D change stats: mean = {avg:.2f}, StdDev = {sd:.4f} ({sd_pct * 100.0:.1f}% of mean)", extra=log_extras
        )
        ...
        if correct_columns_individually:
            ...
            for j in range(len(OHLC)):
                ...
                if appears_suspended and (idx_latest_active is not None and idx_latest_active >= idx_first_f):
                    ...
                    ranges_after = map_signals_to_ranges(
                        f_revj[idx_rev_latest_active:], f_rev_upj[idx_rev_latest_active:], f_rev_downj[idx_rev_latest_active:]
                    )
            ...
            if count == 0:
                ...
                col = OHLC[idx]
                logger.debug(
                    f"Potential {fix_type} detected only in column {col}, so treating as false positive (ignore)", extra=log_extras
                )
            else:
