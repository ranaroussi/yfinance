...
class TickerBase:
    ...
    def _get_fundamentals(self, proxy=None):
        ...
        if isinstance(data.get('earnings'), dict):
            try:
                ...
                earnings['financialCurrency'] = data['earnings'].get('financialCurrency', 'USD')
        ...
        # Complementary key-statistics (currently fetching the important trailingPegRatio which is the
        # value shown in the website)
        res = {}
        try:
    ...
    def get_analysis(self, proxy=None, as_dict=False):
        self._get_fundamentals(proxy=proxy)
        data = self._analysis
        if as_dict:
            return data.to_dict()
