...
class TickerBase:
    ...
    def _reconstruct_intervals_batch(self, df, interval, prepost, tag=-1):
        ...
        for g in dts_groups:
            ...
            weights = _np.tile(weights, len(calib_cols))  # 1D -> 2D
    ...
    def _fix_prices_sudden_change(self, df, interval, tz_exchange, change, correct_volume=False):
        ...
        idx_latest_active = _np.where(f_active & _np.roll(f_active, 1))[0]
        ...
        if interday and interval != '1d':
            # average change
            _1d_change_minx = _np.average(_1d_change_x, axis=1)
        else:
        ...
        if correct_columns_individually:
            ...
            if count == 0:
                ...
                idx = _np.where(_np.array(idxs) != -1)[0][0]
                col = OHLC[idx]
