...
from .exceptions import YFException, YFDataException
...
class Lookup:
    ...
    def _fetch_lookup(self, lookup_type="all", count=25) -> dict:
        ...
        try:
            ...
            msg = f"{self.ticker}: 'lookup' fetch received faulty data"
            self._logger.error()
            if not YfConfig().hide_exceptions:
                raise YFDataException(msg + ":\n> " + str(e))
            data = {}

        # Error returned
        if data.get("finance", {}).get("error", {}):
            error = data.get("finance", {}).get("error", {})
            msg = f"{self.ticker}: 'lookup' fetch returned error: {error}"
            raise YFDataException(msg)
