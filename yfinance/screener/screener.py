...
PREDEFINED_SCREENER_BODY_DEFAULTS = {"offset": 0, "count": 25, "userId": "", "userIdType": "guid"}
...
PREDEFINED_SCREENER_QUERIES = {
    "aggressive_small_caps": {
        ...
        "sortType": "desc",
        "query": EqyQy(
            "and", [EqyQy("is-in", ["exchange", "NMS", "NYQ"]), EqyQy("lt", ["epsgrowth.lasttwelvemonths", 15])]
        ),
    },
    "day_gainers": {
    ...
    "most_shorted_stocks": {
        ...
        "query": EqyQy(
            "and",
            [EqyQy("eq", ["region", "us"]), EqyQy("gt", ["intradayprice", 1]), EqyQy("gt", ["avgdailyvol3m", 200000])],
        ),
    },
    "small_cap_gainers": {
        "sortField": "eodvolume",
        "sortType": "desc",
        "query": EqyQy(
            "and", [EqyQy("lt", ["intradaymarketcap", 2000000000]), EqyQy("is-in", ["exchange", "NMS", "NYQ"])]
        ),
    },
...
@dynamic_docstring(
    {
        "predefined_screeners": generate_list_table_from_dict_universal(
            PREDEFINED_SCREENER_QUERIES, bullets=True, title="Predefined queries (Dec-2024)"
        )
    }
)
def screen(
    ...
    defaults = {"offset": 0, "count": 25, "sortField": "ticker", "sortAsc": False, "userId": "", "userIdType": "guid"}
    ...
    if isinstance(query, str):
        ...
        if size is not None:
            print_once(
                "YF deprecation warning: 'size' argument is deprecated for predefined screens, set 'count' instead."
            )
            count = size
            size = None
